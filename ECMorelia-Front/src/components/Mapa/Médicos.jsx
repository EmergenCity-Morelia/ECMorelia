...
export default function Medicos() {
    ...
    return (
        <div>
                    ...
                    <DataTable value={medicos} responsiveLayout="scroll">
                        ...
                        <Column header="Acciones" body={(rowData) => (
                            <div className="flex">
                                ...
                                <button className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-2 rounded-md w-10 h-10 flex justify-center items-center" onClick={() => deleteMedico(rowData.id)}>
            ...
            <Dialog header={editMode ? "Editar Médico" : "Agregar Médico"} visible={dialogVisible} style={{ width: '50vw' }} onHide={() => setDialogVisible(false)}>
                <div className="p-field mt-2">
                    <label htmlFor="nombre" className="block mb-1">Nombre</label>
                    <InputText id="nombre" value={newMedico.nombre} onChange={(e) => setNewMedico({ ...newMedico, nombre: e.target.value })} className="p-inputtext w-full mb-2"/>
                </div>
                <div className="p-field mt-2">
                    <label htmlFor="especialidad" className="block mb-1">Especialidad</label>
                    <InputText id="especialidad" value={newMedico.especialidad} onChange={(e) => setNewMedico({ ...newMedico, especialidad: e.target.value })} className="p-inputtext w-full mb-2"/>
                </div>
                <div className="p-field mt-2">
                    <label htmlFor="contraseña" className="block mb-1">Contraseña</label>
                    <InputText id="contraseña" value={newMedico.contraseña} onChange={(e) => setNewMedico({ ...newMedico, contraseña: e.target.value })} className="p-inputtext w-full mb-2"/>
                </div>
