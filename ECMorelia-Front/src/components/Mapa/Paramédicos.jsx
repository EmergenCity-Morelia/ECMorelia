...
export default function Paramedicos() {
    ...
    return (
        <div>
                    ...
                    <DataTable value={paramedicos} responsiveLayout="scroll">
                        ...
                        <Column header="Acciones" body={(rowData) => (
                            <div className="flex">
                                ...
                                <button className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-2 rounded-md w-10 h-10 flex justify-center items-center" onClick={() => deleteParamedico(rowData.id)}>
            ...
            <Dialog header={editMode ? "Editar Paramédico" : "Agregar Paramédico"} visible={dialogVisible} style={{ width: '50vw' }} onHide={() => setDialogVisible(false)}>
                <div className="p-field mt-2">
                    <label htmlFor="nombre" className="block mb-1">Nombre</label>
                    <InputText id="nombre" value={newParamedico.nombre} onChange={(e) => setNewParamedico({ ...newParamedico, nombre: e.target.value })} className="p-inputtext w-full mb-2"/>
                </div>
                <div className="p-field mt-2">
                    <label htmlFor="nivel" className="block mb-1">Nivel</label>
                    <InputText id="nivel" value={newParamedico.nivel} onChange={(e) => setNewParamedico({ ...newParamedico, nivel: e.target.value })} className="p-inputtext w-full mb-2"/>
                </div>
                <div className="p-field mt-2">
                    <label htmlFor="contraseña" className="block mb-1">Contraseña</label>
                    <InputText id="contraseña" value={newParamedico.contraseña} onChange={(e) => setNewParamedico({ ...newParamedico, contraseña: e.target.value })} className="p-inputtext w-full mb-2"/>
                </div>

                <div className="p-dialog-footer flex justify-center space-x-2 mt-3">
                    <button className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded" onClick={saveParamedico}>
